...
public class ItemModelScythe extends ItemModel {
    ...
    public ItemModelScythe(Item item, String texturePath, String hueMaskTexturePath) {
        ...
        this.texture = GameTexture.load(texturePath).get();
        this.item = item;
        this.texture = ItemModelBuilder.flip(this.texture);
        if (hueMaskTexturePath != null) {
            this.hueMaskTexture = GameTexture.load(hueMaskTexturePath).get();
    ...
    public void renderGeneric(Vector3 pos, Camera cam, Matrix4 tmpMatrix, boolean isSlot, IItemRenderParams renderParams) {
        if (isSlot) {
            ...
            try {
                GameEntity.setLightingColor(zone, pos, Sky.currentSky.currentAmbientColor, tintColor, tmpBlockPos, tmpBlockPos2);
    ...
    public void render(Vector3 worldPosition, Camera camera, Matrix4 matrix4, boolean useAmbientLight, boolean applyFog, ItemStack itemStack, Color slotColor) {
        itemStackRenderParams.setItemStack(itemStack);
        itemStackRenderParams.setSlotColor(slotColor);
        this.renderGeneric(worldPosition, camera, useAmbientLight ? matrix4 : noRotMat, !useAmbientLight, itemStackRenderParams);
    }
    ...
    public void renderAsItemEntity(Vector3 pos, Camera entityCam, Matrix4 tmpMatrix, ItemGameEntity ie) {
